...
def process_ft(ft_line, A23, image_size, patch_size, npts=256):
    # 输入：
    #     ft_line: list, shape=[1+2+4n]，第一个为类别id，其余为傅里叶系数
    #     patch_size: (W, H)
    #     A23: 2x3仿射矩阵
    # 输出：
    #     yolo_label: YOLO格式字符串（class_id xc yc w h）
    #     ft_line: list，以 class_id 开头的归一化傅里叶系数
    Height,Width = image_size
    W, H = patch_size
    ft = np.array(ft_line, dtype=np.float32)
    ...
    x0 = np.dot(A, coeffs[0:2].reshape(2, 1)) + T
    x0 = x0.flatten()

    if (0 <= x0[0] <= W and 0 <= x0[1] <= H):
        n = (len(coeffs) - 2) // 4
        high_coeffs = coeffs[2:].reshape(n, 4)

        ac = high_coeffs[:, [0, 2]].T
        bd = high_coeffs[:, [1, 3]].T
        ...
        new_coeffs[0:2] = x0
        new_coeffs[2::4] = ac_trans[0]
        new_coeffs[3::4] = bd_trans[0]
        new_coeffs[4::4] = ac_trans[1]
        new_coeffs[5::4] = bd_trans[1]

        # 重建闭合曲线
        x,y = fft_1(new_coeffs,npts)
        ...
        ft_norm = np.copy(new_coeffs)
        ft_scale(ft_norm,(1.0/W,1.0/H))
        ft_line = [class_id] + ft_norm.tolist()

        #xy(x,y)->p[2,npts]->p[npts,2]
        pts = np.vstack(list((x,y))).T #p[2,npts]->p[npts,2]

        return yolo_label, ft_line, pts
    else:
...
def read_image_label(image_path):
    ...
    if os.path.exists(ft_path):
        with open(ft_path, "r") as f:
            ft_data = [list(map(float, line.strip().split())) for line in f.readlines() if len(line.strip().split()) >= 7]
        assert txt_data == None or len(ft_data) == len(txt_data)
    else:
        ft_data = None

    return image, txt_data, pol_data, ft_data

def save_patch(image, new_pol_data, label_txt, label_pts, patch_name, output_path):
    ...
    ft_name = f"{output_path}/labels/{patch_name}.ft"
    ...
    if label_pts is not None and len(label_pts)>0 and label_pts[0]is not None:
        with open(ft_name, "w") as f:
...
def generate_patches(data_path, diz, diz_angle, diz_scale, min_dist_to_edge, npts, max_obj_select, max_neg_select,output_folder='patches',max_generated=0,draw_polys_on_big=0):
    ...
    for image_path in tqdm(image_files, total=len(image_files)):
        image, txt_data, pol_data, ft_data = read_image_label(image_path)
        if image is not None and (pol_data is not None or (txt_data is not None and ft_data is not None)):
            ...
            while True:
                ...
                if subset:
                    ...
                    new_txt_data, new_ft_data, new_pol_data = [], [], []
                    assert pol_data is None or ft_data is None or len(pol_data)==len(ft_data)
                    if pol_data is not None:
                        for j, pol_line in enumerate(pol_data):
                            ft_line = ft_data[j] if ft_data is not None else None
                            ...
                            if pol_line is not None:
                                ...
                                if min(xy[0],PATCH_SIZE[0]-xy[0],xy[1],PATCH_SIZE[1]-xy[1]) >  min_dist_to_edge:
                                    select_times[j] += 1
                    elif ft_data is not None:
                        for j, ft_line in enumerate(ft_data):
                            pol_line = pol_data[j] if pol_data is not None else None
                            yolo_label, ft_line, pts = process_ft(ft_line, A23, (height,width), PATCH_SIZE, npts=npts)
                    ...
                    save_patch(patch, new_pol_data, new_txt_data, new_ft_data, patch_name, output_path)
                    patch_index += 1
                else:
                    break
            ...
            if draw_polys_on_big > 0:
                if pol_data is not None:
                    ...
                    for j, ft_line in enumerate(ft_data):
                        ...
                        #xy(x,y)->p[2,npts]->p[npts,2]
                        pts = np.vstack(list(xy)).T #p[2,npts]->p[npts,2]
                        pts = pts.reshape((-1, 1, 2)).astype(np.int32) #pts[npts,2]->pts[npts,1,2]
                        cv2.polylines(image,[pts],True,colors[class_id%len(colors)],thickness=2)
...
if __name__ == "__main__":
    ...
    data_path = '/media/liu/f4854541-32b0-4d00-84a6-13d3a5dd30f2/datas/dota1.5/isAID-hull3/train' #
    total_train = generate_patches(data_path, 36, 180, 1.1, MIN_DISTANCE_TO_EDGE,
                                   256, 3, 1,output_folder='patches',max_generated=0,draw_polys_on_big=10)
    
    data_path = '/media/liu/f4854541-32b0-4d00-84a6-13d3a5dd30f2/datas/dota1.5/isAID-hull3/val' #
    total_val = generate_patches(data_path, 36, 180, 1.1, MIN_DISTANCE_TO_EDGE,
                                 256, 1, 1,output_folder='patches',max_generated=0,draw_polys_on_big=10)
