...
# 3. 在图像上填灰色区域
def fill_polys_on_image(image, subset):
    gray_color = 128 if len(image.shape) == 2 else (128, 128, 128)
    for poly in subset:
        cv2.fillPoly(image, [poly.astype(np.int32).reshape(4, 1, 2)], color=gray_color)
    return image


def process_image(img_path, output_dir, diz=30, diz_angle=180, CROP_SIZE = (640, 640), MAX_SELECTION_PER_OBJ=1, least_len=3, change_rate=0.2):
...
if __name__ == "__main__":
    ...
    # Paths
    data_path = '/data4T/datas/dota1.5' # #"images"
    ...
    for img_file in tqdm(image_files):
        ...
        process_image(img_path, output_dir, diz = diz, diz_angle=180, CROP_SIZE = CROP_SIZE, MAX_SELECTION_PER_OBJ=MAX_SELECTION_PER_OBJ, least_len=3)
    # 写入类别名到 names.txt
