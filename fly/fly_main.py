...
def draw_map_with_uavs(thumbnail_surface, map_shape, q, crop_size, current_t, current_p, idx):
    ...
    if current_t[0] is not None and current_t[1] is not None:
        ...
        # 绘制四边形
        pygame.draw.polygon(thumbnail_surface, (255, 255, 255), t_mapped, 1)   # 绘制 cur_cams 的四边形，红色
        ...
        text_rect = text_surface.get_rect(center=(int(p_center_mapped[0]), int(p_center_mapped[1])))
...
if __name__ == '__main__':
    ...
    QUEUE_LIMIT = 4
    MAX_SCORE_LIMIT = 256
    config['images_dir'] = replace_path(config['images_dir'])
    ...
    config['model_best_path'] = replace_path(config['model_best_path'])
    if config['model_best_path'] and os.path.exists(config['model_best_path']):
        ...
        while running and frame_count < 99999999:
            ...
            if image is not None:
                ...
                for idx, det in enumerate(pred):  # per image batch循环
                    ...
                    if len(det): #det[np,4(pts)*2+1(conf)+1(cls)]
                        ...
                        A23t = cur_cams[idx][-1] #(t_xc, t_yc, t_cos, t_sin, A23t)
                        A23_inv = A23inverse_torch(A23t)
                        if mname in [0, 1]:
            ...
            #         thumbnail_objects[:, 0] += pad_x
            #         thumbnail_objects[:, 1] += pad_y
            #         draw_oriented_fan(thumbnail_objects, globe_surface)
                    
            #         thumbnail_objects[:, [0, 2]] = thumbnail_objects[:, [0, 2]] * scale_hot[0]
            #         thumbnail_objects[:, [1, 3]] = thumbnail_objects[:, [1, 3]] * scale_hot[1]
            #         batched_multivariate_normal_pdf(xy_grid.to(device), thumbnail_objects, thumbnail_size), (0,0)
            ...
            for idx, fly in enumerate(flys):
                thumbnail_surface = draw_map_with_uavs(thumbnail_surface, map[0], flys_q[idx], CROP_SIZE, cur_cams[idx], None, idx)
            ...
            #scores surface
            if globe_scores.shape[0]>1:
                score_width, score_height = 300, 50
                # screen = pygame.display.set_mode((width, height))
                plot_surface = pygame.Surface((score_width, score_height), pygame.SRCALPHA)

                # 获取需要绘制的两条曲线的数据（红色、蓝色）
                red_values = globe_scores[:, 0]
                blue_values = globe_scores[:, 1]
                # 构造坐标点列表 [(0, y0), (1, y1), ..., (T-1, yT-1)]
                time_scale = score_width / MAX_SCORE_LIMIT
                y0 = score_height / 2
                score_scale = y0 / (0.01 + torch.max(torch.max(red_values),torch.max(blue_values)))
                red_points = [(int(time_scale * x), int(y0 - score_scale * red_values[x])) for x in range(red_values.shape[0])]
                blue_points = [(int(time_scale * x), int(y0 + score_scale * blue_values[x])) for x in range(blue_values.shape[0])]

                plot_surface.fill((0, 0, 0, 0))  # black底
                pygame.draw.line(plot_surface, (255, 255, 0), (0,y0), (score_width,y0), 2)
                pygame.draw.lines(plot_surface, (255, 0, 0), False, red_points, 1)   # 红色曲线
                pygame.draw.lines(plot_surface, (0, 0, 255), False, blue_points, 1)  # 蓝色曲线

                #
                text = f"{num_side[0]} vs {num_side[1]}"
                # 渲染成 surface（yellow字体）
                text_surface = font.render(text, True, (255, 255, 0))
                # 获取文字的矩形，并设置中心为 p_center_mapped
                text_rect = text_surface.get_rect(topleft=(10, y0-25))
                # 绘制文字到 thumbnail_surface
                plot_surface.blit(text_surface, text_rect)
                screen.blit(plot_surface, (report_surface.get_width() + 10, thumbnail_surface.get_height()))

                
                plot_surface = pygame.Surface((score_width, score_height), pygame.SRCALPHA)
                score3_values = globe_scores[:, 2]
                time_scale = score_width / MAX_SCORE_LIMIT
                y0 = score_height / 2
                score_scale = y0 / (0.01 + torch.max(score3_values.abs()))
                score3_points = torch.stack([time_scale * torch.arange(score3_values.shape[0], device=device), score_scale * score3_values], dim=-1)
                red_points = score3_points.cpu().numpy()
                blue_points = red_points.copy()
                red_points[:, 1] = y0 - np.clip(red_points[:, 1], 0, np.inf)
                blue_points[:, 1] =  y0 - np.clip(blue_points[:, 1], -np.inf, 0)

                plot_surface.fill((0, 0, 0, 0))  # black底
                pygame.draw.lines(plot_surface, (255, 0, 0), False, red_points.astype(int), 1)   # 红色曲线
                pygame.draw.lines(plot_surface, (0, 0, 255), False, blue_points.astype(int), 1)  # 蓝色曲线
                pygame.draw.line(plot_surface, (255, 255, 0), (0,y0), (score_width, y0), 2)
                screen.blit(plot_surface, (report_surface.get_width() + 10, thumbnail_surface.get_height() + 2 * score_height))
