...
def reload(map_name, device):
    map = [None, None]
    if os.path.splitext(map_name)[1].lower()=='.tif':
        map[0] = read_tif_with_tifffile(map_name)
    else:
        map[0] = cv2.imread(map_name, cv2.IMREAD_COLOR)
    if map[0] is None:
        raise ValueError(f"Failed to load image: {map_name}")
    map[1] = torch.from_numpy(map[0]).float().permute(2, 0, 1).unsqueeze(0).to(device)  # [H,W,C] -> [1, C, H, W]
    return map
...
class FlyCrop:
    ...
    def render(self, mA=None):
        if self.map is None:
            return None
        mA = self.mA if mA is None else mA
        frame = kornia.geometry.transform.warp_affine(self.map_t, mA.float(), dsize=self.size_grab)
        return frame #frame[1,C,H,W]
    
    def bump(self):
        hit_edge = 0
        width, height = self.map.shape[1], self.map.shape[0]
    ...
    def set_affine(self, A23):
        self.mA[0] = torch.from_numpy(A23.reshape(2, 3))