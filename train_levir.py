    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, ioa, samples) in pbar:  # batch--> imgs[b,C,H,W]  targets[ntb,10=1(b)+1(cls)+4(npts)*2(xy)]
            ...
            with autocast(enabled=amp_flag): # enabled=cuda
                #img[b,c,h,w]
                pabuv, pred = model(imgs)  # forward   model(W,imgs)=pA23s[b,2,3],pred(obb) ~ targets
...
def parse_opt(known=False):
    ...
    #whu-ft
    opt.data = 'data/LEVIR-CD-max_eq_5.yaml' #'data/LEVIR-CD-nohyp.yaml' #'data/Google-CD.yaml' #'data/LEVIR-CD.yaml' #'data/whu-diff3.yaml' #'data/WHU-hull_ft-dif.yaml'
    #whu
    # opt.data = 'data/whu-diff.yaml'

    # opt.cfg = 'models/yolov11s-dif-VitA23.yaml'
    opt.cfg = 'models/yolov11s-dif-HA23.yaml'
    ...
    opt.epochs = 200
    opt.device = 'cuda:1'
    #opt.resume = 'runs/train/LEVIR-CD-yolov11s-dif-HA23-bs16-50.84-35.93-epo129-2-1280x1280/weights/last.pt'
