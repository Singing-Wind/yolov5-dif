...
def model_info(model, verbose=False, img_size=640):
    ...
    try:  # FLOPs
        ...
        stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 128
    ...
    LOGGER.info(f"\033[92mModel Summary: {len(list(model.modules()))} layers, {n_p} parameters, {n_g} gradients{fs}\033[0m")
