...
from DOTA_devkit.polyiou_cpu import poly_nms_cpu64, poly_iou_cpu64
...
def big_nms(det_list, scores, iou_thres):
    if det_list.shape[1]==8:
        y = torch.cat((det_list, scores[:,None]), dim=1)
        y = y.cpu().numpy().astype(np.float64)
        # i = py_cpu_nms_poly_fast(y, iou_thres)
        i = poly_nms_cpu64(y, iou_thres)
    else: #det_list.shape[1]==4
        assert det_list.shape[1]==4
        boxes = det_list[:, :4] # boxes[nt,4] (offset by class), scores[nt]
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300, save_nms_boxes=False):
    ...
    to_save_array = []  # [n_imgs, n_boxes, xyxy=4 + obj_conf=1 + class=1 + mns_label=1]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if save_nms_boxes:
            # 保存boxes训练数据
            nms_labels = np.zeros(n)   # x.shape[0] = n_boxes
            nms_labels[i] = 1
            nms_labels = np.expand_dims(nms_labels, axis=1)
            nms_labels = torch.tensor(nms_labels)
            img_boxes_data = torch.cat((x[:, :4], torch.unsqueeze(scores, axis=1), x[:, 5:6], nms_labels), dim=1)
            to_save_array.append(img_boxes_data)
