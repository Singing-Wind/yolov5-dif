...
from models.yolo_try_vit_a23_2 import Model, OUT_LAYER
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, imgsz = \
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=6, nc=nc, imgsz=imgsz, anchors=hyp.get('anchors')).to(device)  # create
        ...
        model = Model(cfg, ch=6, nc=nc, imgsz=imgsz, anchors=hyp.get('anchors')).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, ioa, samples) in pbar:  # batch--> imgs[b,C,H,W]  targets[ntb,10=1(b)+1(cls)+4(npts)*2(xy)]
            ...
            with autocast(enabled=amp_flag): # enabled=cuda
                #img[b,c,h,w]
                (pabuv, pred), theta = model(imgs, i=i)  # forward   model(W,imgs)=pA23s[b,2,3],pred(obb) ~ targets
                ...
                if dfl_flag: #gtA23s[B]
                    ...
                    lambda_theta = 0.4
                    loss_theta = F.smooth_l1_loss(theta, samples['A23']) * lambda_theta
                    loss = loss + loss_theta
            ...
            def print_gradients(model):
                for name, param in model.named_parameters():
                    if param.grad is not None:
                        print(f"{name:30s} grad mean: {param.grad.mean():.6f}, max: {param.grad.max():.6f}")

            
            print_gradients(model)  # 打印梯度状态
...
def parse_opt(known=False):
    ...
    #whu-ft
    opt.data = 'data/whu-diff6.yaml' #'data/Google-CD.yaml' #'data/LEVIR-CD.yaml' #'data/whu-diff3.yaml' #'data/WHU-hull_ft-dif.yaml'
    #whu
    # opt.data = 'data/whu-diff.yaml'

    # opt.cfg = 'models/yolov11s-dif-VitA23.yaml'
    opt.cfg = 'models/yolov11s-dif-HA23-first-match.yaml'
