...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.patch_embed(x)  # x[b, hw, 768]

        x = self.pos_drop(x + self.pos_embed) # x[b, hw, 768]
        x = self.encoder_block(x) # x[b, hw, 768]
        x = self.norm(x) # x[b, hw, 768]
        return x # x[b, hw, 768]
...
class SplitViT(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = x[:,:3]
        x1 = x[:,3:]
        x0_out = self.VIT1(x0).transpose(-1,-2).reshape(B, self.embed_dim, self.grid_size[0], self.grid_size[1]) # x0_out[b, hw, 768]->x0_out[b, 768, h, w] 如[b, 768, 40, 40]
...
class MLPHead(nn.Module):
    ...
    def __init__(self, embed_dim=768*2, out_dim=6, hidden_dim=None, init_identity=True, delta_scale=0.3, max_trans=0.5):  # delta_scale=0.05, max_trans=0.2
        super().__init__()
        ...
        if init_identity:
            ...
            with torch.no_grad():
                ...
                self.fc2.bias.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))
