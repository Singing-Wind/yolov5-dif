...
from .yolo_base import DetectDFL,OBB, DetectDFL_xn, OBB_xn
from .HA23 import HA23
from .VitA23 import VitA23
from .VIT_first_match2 import SplitViT, MLPHead, SpatialAffineSampler
from .LocAtt import LocAtt,LocAttGrouped
from torch import autocast  # 新版接口
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=6, nc=None, imgsz=640, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], imgsz=imgsz)  # model, savelist
    ...
    def forward(self, x, augment=False, profile=False, visualize=False, i=60):
        #CwCwCwCw
        if CwCwCwCw:
            # 原始输入x的形状: [B,C,H,W]
            b, C, h, w = x.shape
        ...
        return self.forward_once(x, profile, visualize, i)  # single-scale inference, train
    ...
    def forward_once(self, x, profile=False, visualize=False, i=60):
        y, dt = [], []  # outputs
        img0 = x  # [bs, 6, 640, 640]
        for m in self.model:
            ...
            # 这里判断是否是 ViT 模块
            if isinstance(m, SplitViT):
                # 仅 ViT 用混合精度
                with autocast(device_type="cuda", dtype=torch.float16):  #torch.cuda.amp.autocast显示已弃用
                    x = m(x)
            else:
                # 其他层用正常 FP32
                if m.i == 2:
                    
                    if isinstance(m, SpatialAffineSampler):
                         x, theta = m(img0, x, i)
                    else:
                        x = m(img0, x, i)
                else:
    ...
    def get_module_byname(self, name:str):
        if not hasattr(self, 'module_idx'):
            ...
            for i, h in enumerate(self.yaml['head']):
                if h[2] in OUT_LAYER: #h[2]就是module名字（head部分的，如OBB）
                    self.module_idx[h[2]] = i + len(self.yaml['backbone']) + len(self.yaml['preproc']) if self.yaml.get('preproc', False) else i + len(self.yaml['backbone'])
...
def parse_model(d, ch, imgsz):  # model_dict, input_channels(3)
    ...
    strides = [1]
    for i, (f, n, m, args) in enumerate(d['preproc'] + d['backbone'] + d['head']):  # from, number, module, args
        f = f+len(d['preproc']) if (isinstance(f, int) and f!=-1) else f
        if isinstance(f, int):
            if f!=-1:
                f += len(d['preproc'])
        else:
            f = [i+len(d['preproc']) if i!=-1 else i for i in f]
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
        ...
        elif m in [Detect, DetectDFL, OBB, YoloText, OBBText, DetectDFL_xn, OBB_xn]:
            args.append([ch[x] for x in f])
            if m not in [DetectDFL, OBB, YoloText, OBBText, DetectDFL_xn, OBB_xn]:
            ...
            c2 = ch[f] // args[0] ** 2
        elif m in {HA23,VitA23}:
            args.append([ch[x] for x in f[:-1]])
            args.append([strides[x] for x in f[:-1]])
        elif m is SplitViT:
            c1, c2 = ch[f], args[0]
            args = [imgsz, c1, c2, *args[1:]]
        elif m is MLPHead:
            c1, c2 = ch[f], args[0]  # 原为ch[f]*2
            args = [c1, c2, *args[1:]]
        elif m is SpatialAffineSampler:
            c2 = 6
        else:
            c2 = ch[f]
        ...
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)  #save.extend(x % i + len(d['preproc']) for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist    # save只是定义模型在前向传播中要保存的层，方便跨层连接。本身Parse_model只定义模块而非连接关系。 #changed by xz: just x%i before, now: len(self.yaml['preproc'])=3
        layers.append(m_)
        if i == 0:
            ch = []
        ch.append(c2)
