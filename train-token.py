...
from CLIP.CLIPDetectionModel import CLIPDetectionModel
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Model
    config_path = 'config-clip.json' #
    config = load_config(config_path)
    model = CLIPDetectionModel(block_size=config.get('block_size',1),num_classes=nc,np_max=data_dict.get('np_max',100)) #CLIPCaptionModel()
    pretrained = weights.endswith('.pt')
    if pretrained:
        if os.path.exists(weights):
            start_epoch = load_checkpoint(weights,model,optimizer,scheduler)
        else:
            start_epoch = 0
        # # with torch_distributed_zero_first(RANK):
    ...
    model.set_nc(len(dataset.names),device=device)
    ...
    nl = 3 #m.nl  # number of detection layers (to scale hyps)
    if not dfl_flag:
    ...
    model.class_weights = labels_to_class_weights(master_labels, nc).to(device) * nc #class_weights[nc] attach class weights
    model.names = names
    ...
    if dfl_flag:
        amp_flag = cuda #check_amp(model, LOGGER)
        scaler = (
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, ioa, samples) in pbar:  # batch--> imgs[b,C,H,W]  targets[ntb,1(b)+1(cls)+4(xywh)]
            ...
            imgs = imgs.to(device, non_blocking=True).float() / 255.0 #imgs[B,C,H,W] uint8 to float32, 0-255 to 0.0-1.0
            ...
            with autocast(enabled=amp_flag): # enabled=cuda
                #img[b,c,h,w]
                preds = model(imgs,targets.to(device))  # forward   model(W,imgs)=pred ~ targets
                #pred{x[nl=3][B,C=4*16+nc,H,W] angle[B,1,ntotal]}
                #targets[n_batch,1+1+4]#注意n_batch是一个batch里所有目标的总集，因此1(batch编号)+1(目标类别)+4(目标框坐标)
                loss, loss_items = compute_loss(preds, targets.to(device), imgs.shape[2:], ioa.to(device), samples, imgs = imgs)  # loss scaled by batch_size
                if RANK != -1:
...
def parse_opt(known=False):
    ...
    opt.batch_size = 16
    #coco2017
    # opt.data = 'data/coco2017-ft.yaml'
