...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    #     from tools.rename_exp import process_folder
    #     process_folder('runs/train')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(4 + (mtext is not None), device=device)  # mean losses
        if RANK != -1:
            train_loader.sampler.set_epoch(epoch)
        pbar = enumerate(iter(train_loader))
        if dfl_flag:
            if mtext is None:
                tags = ('Epoch', 'gpu_mem', 'box', 'cls', 'dfl', 'a23','labels', 'img_size')
            else:
        ...
        with warnings.catch_warnings():
            ...
            # Scheduler
            scheduler.step()
        for i, (imgs, targets, paths, _, ioa, samples) in pbar:  # batch--> imgs[b,C,H,W]  targets[ntb,10=1(b)+1(cls)+4(npts)*2(xy)]
            ...
            assert len(gtA23s)==imgs.shape[0]
            assert gtA23s.shape==(imgs.shape[0],2,3)
            ...
            def transform_targets_back(target, A23):
                """
                target: [n_targets, 10] -> batch_idx, cls, x1..x4, y1..y4
                A23: [B,2,3] -> 仿射矩阵 Img0->Img1
                """
                device = target.device
                batch_idx = target[:,0].long()  # [n_targets]

                # 取坐标 [n_targets, 4, 2]
                coords = target[:, 2:].reshape(-1, 4, 2)  # [n_targets, 4, 2]

                # 构造齐次坐标 [n_targets, 4, 3]
                ones = torch.ones(coords.shape[0], coords.shape[1], 1, device=device)
                coords_h = torch.cat([coords, ones], dim=-1)  # [n_targets,4,3]

                # 对应 batch 的 A23 取逆
                A23_inv = torch.zeros(A23.shape, device=device)
                # 构造 [B,3,3]
                A_full = torch.zeros(A23.shape[0], 3, 3, device=device)
                A_full[:, :2, :] = A23
                A_full[:, 2, 2] = 1.0
                A_inv_full = torch.inverse(A_full)  # [B,3,3]
                A23_inv = A_inv_full[:, :2, :]      # [B,2,3]

                # 按 batch_idx 对应的 A23_inv
                A_selected = A23_inv[batch_idx]  # [n_targets,2,3]

                # 转换坐标
                coords_trans = torch.bmm(coords_h, A_selected.transpose(1,2))  # [n_targets,4,2]

                # 拼回 target
                target_transformed = target.clone()
                target_transformed[:, 2:] = coords_trans.reshape(-1, 8)
            ...
            with autocast(enabled=amp_flag): # enabled=cuda
                #img[b,c,h,w]
                (pabuv, pred),pred0 = model(imgs)  # forward   model(W,imgs)=pA23s[b,2,3],pred(obb) ~ targets
                # assert pA23s.shape==gtA23s.shape
                ...
                if dfl_flag: #gtA23s[B]
                    loss, loss_items = compute_loss(pabuv, pred, targets.to(device), imgs.shape[2:], ioa.to(device), samples, imgs = imgs)  # loss scaled by batch_size
                    target_back = transform_targets_back(targets, samples['A23'])

                    loss0, loss_items0 = compute_loss(pabuv, pred0, target_back.to(device), imgs.shape[2:], ioa.to(device), samples, imgs = imgs)
                    loss += loss0
                else:
                    loss, loss_items = compute_loss(pabuv, pred, targets.to(device), paths=paths, master_path=master_path)  # loss scaled by batch_size
        ...
        if RANK in [-1, 0]:
            ...
            if epoch >= 0:  # 原为>=data_dict.get('val_epoch', 0) and (not noval or final_epoch):  # Calculate mAP
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if time.time() - last_save_time > 300:  # 300秒 = 5分钟
...
def parse_opt(known=False):
    ...
    devices,device_total = get_available_cuda_devices(min_memory_gb=15, max_memory_usage=30, max_gpu_util=15)
    if len(devices)>0:
        if(device_total > 1):
    ...
    opt.imgsz = [640*2, 640*2]
    #opt.epochs = 100
    opt.batch_size = 8
    
    #dota
    # opt.data = 'data/dota-dif.yaml'
    # opt.data = 'data/dota-sim.yaml' #noise+blur
    #whu-ft
    opt.data = 'data/whu-diff7.yaml' #'data/Google-CD.yaml' #'data/LEVIR-CD.yaml' #'data/whu-diff3.yaml' #'data/WHU-hull_ft-dif.yaml'
    #whu
    # opt.data = 'data/whu-diff.yaml'

    # opt.cfg = 'models/yolov11s-dif-VitA23.yaml'
    opt.cfg = 'models/yolov11s-dif-HA23-OBB2.yaml'  #🌟'models/yolov11s-dif-HA23.yaml'
    # opt.cfg = 'models/yolov12s-obb.yaml'
    # opt.cfg = 'models/yolov11n-obb-LA.yaml'
    opt.weights = ''
    #'../models/dota1.5/dota-diff-yolov11s-dif-obb-bs16-99.50-94.61-epo299/weights/best.pt'
    #'../../weights/yolov11/yolov11s.pth'#'weights/yolov11s.pt' #'weights/yolov5m.pt'
    #'/home/liu/workspace/yolov5/yolov5-dif/models/dota1.5/dota-diff-yolov11s-dif-obb-bs16-99.50-98.89-epo99-640x640/weights/best.pt'
    opt.hyp = 'data/hyps/hyp.scratch-dfl-dif.yaml'
    opt.epochs = 200
    opt.device = 'cuda:0'
